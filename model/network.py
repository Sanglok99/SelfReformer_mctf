...
class Net(nn.Module):
    def __init__(self, opt):
        ...
        self.transformer = nn.ModuleList([Transformer(depth=d,
                                                      ...
                                                      mlp_ratio=m,
                                                      num_patches=p,
                                                      opt=opt) for d, n, e, m, p in opt.transformer])
        ...
        self.CRM_7 = CRM(inc=512, outc=1024, hw=7, embed_dim=512, num_patches=49, opt=opt)
        self.CRM_14 = CRM(inc=320, outc=256, hw=14, embed_dim=320, num_patches=196, opt=opt)
        self.CRM_28 = CRM(inc=128, outc=64, hw=28, embed_dim=128, num_patches=784, opt=opt)
        self.CRM_56 = CRM(inc=64, outc=16, hw=56, embed_dim=64, num_patches=3136, opt=opt)
    ...
    def forward(self, x):
        ...
        # print("shape of out after encoder_tf_ss:", out.shape) ### [1, 168, 256](r_eval 14일 때), [1, 156, 256](r_eval 20일 때) 
            ### 168 = 196 - 14*2, 156 = 196 - 20*2 (=> encoder_tf_ss는 depth=2의 transformer이기 때문에 두 번의 token fusion 일어남)
            ### encoder_tf_ss에는 token fusion 적용하면 안 될듯
        ...
        ### print("shape of matt after pixel shuffle:", matt.shape) ### [1, 1, 224, 224]
        ...
        # ----------------------------for SOD
        out_7, out_14, out_28, out_56 = [tf(o, peb) for tf, o, peb in zip(self.transformer, ### nn.ModuleList <- [2, 1, 512, 3, 49], [2, 1, 320, 3, 196], [2, 1, 128, 3, 784], [2, 1, 64, 3, 3136]
                                                                          [out_7r, out_14r, out_28r, out_56r], ### [1, 49, 512], [1, 196, 320], [1, 784, 128], [1, 3136, 64]
                                                                          [False, False, False,
