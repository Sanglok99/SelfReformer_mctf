...
### 입력: x = (B, N, C), attn = (B, N, C)
def mctf(x, attn, opt, r, train = False): 
    if r == 0: # r이 0일 때
        raise ValueError
    else:
        t = x.shape[1]
        ...
        r = min(int((x.shape[1]) * r) if r < 1 else r, (x.shape[1]) // 2)

        if r == 0:
            return x, False

        if C == 512: # out_7 mctf
            merge, _ = bipartite_soft_matching(x, attn = attn, r = r, 
                                            tau_sim       = opt.tau_sim,
                                            tau_info      = opt.tau_info,
                                            tau_size      = opt.tau_size,
                                            size          = opt.size_7,
        ...
        if opt.trace_source:
            opt.source = merge_source(
                merge, x, opt.source
            )

        if C == 512: # out_7 mctf merge
            x, opt.size_7, attn_n = merge_wavg(merge, x, opt.size_7, attn, one_step_ahead=opt.one_step_ahead, pooling_type=opt.pooling_type)
        elif C == 320: # out_14 mctf merge
            x, opt.size_14, attn_n = merge_wavg(merge, x, opt.size_14, attn, one_step_ahead=opt.one_step_ahead, pooling_type=opt.pooling_type)
        elif C == 128: # out_28 mctf merge
            x, opt.size_28, attn_n = merge_wavg(merge, x, opt.size_28, attn, one_step_ahead=opt.one_step_ahead, pooling_type=opt.pooling_type)
        elif C == 64: # out_56 mctf merge
            x, opt.size_56, attn_n = merge_wavg(merge, x, opt.size_56, attn, one_step_ahead=opt.one_step_ahead, pooling_type=opt.pooling_type)
        else:
            x, opt.size, attn_n = merge_wavg(merge, x, opt.size, attn, one_step_ahead=opt.one_step_ahead, pooling_type=opt.pooling_type)
...
def bipartite_soft_matching(
    ...
    with torch.no_grad():
        ...
        a, b = metric[..., ::2, :], metric[..., 1::2, :]  # (12, 99, 64), (12, 98, 64) ### (B, T/2, D)
        ### 짝수 인덱스와 홀수 인덱스로 분리

        if tau_sim:
            W_sim = a @ b.transpose(-1, -2) ### (12, 99, 64) @ (12, 64, 98) = (12, 99, 98)
            W_sim = ((W_sim + 1) / 2) ** (1 / tau_sim) ### (12, 99, 98)
        else:
        ...
        node_max, node_idx = scores.max(dim=-1)  # (12, 99), (12, 99) ### (B, T/2)
        edge_idx = node_max.argsort(dim=-1, descending=True)[..., None]  # (12, 99, 1) ### (B, T/2, 1)
